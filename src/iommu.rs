...
mod tests {
    ...
    use crate::bitmap::{AtomicBitmap, NewBitmap};
    use crate::{
        Address, Bytes, GuestAddress, GuestMemoryError, GuestMemoryMmap, GuestMemoryRegion,
        GuestMemoryResult, IoMemory, Iommu, IommuMemory, Iotlb, Permissions,
    };
    use std::fmt::Debug;
    ...
    fn check_virt_mem_content(
        ...
        // First try the `try_access()` interface
        ...
        // Next try the `get_slices()` interface
        ref_value = value_offset;
        for slice in mem.get_slices(start, len, Permissions::Read).unwrap() {
    ...
    fn test_dirty_bitmap() {
        ...
        // Verify that `try_access()` dirties the bitmap when reporting write access
        mem.try_access(PHYS_LEN, VIRT_START, Permissions::Write, |ofs, _, _, _| {
            // only report one page to be written
            if ofs == 0 {
                Ok(PAGE_SIZE)
            } else {
                assert_eq!(ofs, PAGE_SIZE);
                Ok(0)
            }
        })
        .inspect_err(|err| panic!("Simulated write to memory failed: {err}"))
        .unwrap();
        verify_mem_bitmap(&mem, [PHYS_PAGE_0, PHYS_PAGE_1, VIRT_PAGE_1], [VIRT_PAGE_0]);

        // And test simulated writing to the whole area
        mem.try_access(PHYS_LEN, VIRT_START, Permissions::Write, |_, _, _, _| {
            Ok(PHYS_LEN)
        })
        .inspect_err(|err| panic!("Simulated write to memory failed: {err}"))
        .unwrap();
